...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/artelab_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/artelab_db
...
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3006/api/producto/507f1f77bcf86cd799439011/upload-image
   ```
...
**Imagen original:**
```
http://localhost:3006/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3006/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
# 2. Crear Producto
PRODUCTO_ID=$(curl -X POST http://localhost:3006/api/producto \
...
# 3. Subir imagen
curl -X POST http://localhost:3006/api/producto/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3006/uploads/`
...
- **Categoria**: `POST /api/categoria/{id}/upload-image`
...
```bash
curl -X GET http://localhost:3006/api/cliente-profile/me \
...
```bash
curl -X PUT http://localhost:3006/api/cliente-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `artelab-api.postman_collection.json`
...
├── artelab-api.postman_collection.json
└── package.json
```
